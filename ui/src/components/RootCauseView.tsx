...
export const RootCauseView: React.FC<RootCauseViewProps> = ({
  ...
  const [bottomPanelHeight, setBottomPanelHeight] = useState<number>(320); // h-80 = 320px
  ...
  if (!analysis || !analysis.incident || !analysis.incident.graph || analysis.incident.graph.nodes.length === 0) {
    console.error('[RootCauseView] Invalid analysis data:', analysis);
    return (
  ...
  // Clear loading state when analysis data changes
  useEffect(() => {
    setIsLoading(false);
  }, [analysis]);
  ...
  const layoutResult: LayoutResult | null = useMemo(() => {
    try {
      ...
      console.log('[RootCauseView] Layout result:', result ? `${result.nodes.length} nodes positioned` : 'null');
      return result;
    } catch (error) {
  ...
  // Removed auto-reset on layout computation - user should manually reset if needed
      ...
      <div className="h-14 border-b border-[var(--color-border-soft)] px-6 flex items-center justify-between bg-[var(--color-surface)]">
        ...
        {layoutResult && (
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                // Reset zoom and pan to identity (no zoom, no pan)
                const newTransform = d3.zoomIdentity;
                setTransform(newTransform);
                if (zoomRef.current && graphContainerRef.current) {
                  d3.select(graphContainerRef.current).call(zoomRef.current.transform, newTransform);
                }
              }}
              className="px-3 py-1 text-xs bg-[var(--color-surface-muted)] hover:bg-[var(--color-border-soft)] rounded text-[var(--color-text-primary)] transition-colors"
