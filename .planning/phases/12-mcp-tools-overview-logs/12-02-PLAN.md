...
<objective>
Implement MCP tools for Logz.io progressive disclosure (overview → logs).
...
6. Validate query params: call ValidateQueryParams to reject leading wildcards (though tool schema doesn't expose raw query field - this is defensive)
...
- Tool schemas expose time range, namespace, and filter parameters
