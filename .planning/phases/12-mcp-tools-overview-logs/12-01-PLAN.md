...
must_haves:
  truths:
    ...
    - "Internal regex patterns validated to prevent leading wildcard performance issues"
  artifacts:
...
- ValidateQueryParams(params QueryParams) error:
  ...
  - **NOTE:** This validation is for internal use by aggregation queries, NOT for user-exposed parameters (logs tool doesn't expose regex field to users)
...
- TestValidateQueryParams_LeadingWildcard: Verify rejection of *prefix and ?prefix patterns (validates internal severity patterns)
...
- ValidateQueryParams rejects leading wildcard queries (protects overview tool's internal severity regex)
...
7. **Validation purpose:** Confirm ValidateQueryParams validates internal regex patterns (used by overview tool severity detection), not user-exposed parameters
</verification>
...
- ValidateQueryParams rejects leading wildcard queries with helpful error (validates internal severity patterns)
...
- ValidateQueryParams purpose clarified (internal regex validation for severity detection)
