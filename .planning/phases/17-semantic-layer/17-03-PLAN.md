...
4. Update call sites in `dashboard_syncer.go`:
   - Find where GraphBuilder is created (line 51: `graphBuilder: NewGraphBuilder(graphClient, logger)`)
   - Pass integration config to NewGraphBuilder
   - Example: `graphBuilder: NewGraphBuilder(graphClient, syncer.integration.config, logger)`
...
Verify call site updated: `grep -n "NewGraphBuilder" internal/integration/grafana/dashboard_syncer.go`
...
- dashboard_syncer.go updated to pass config
...
3. **Integration complete:**
   ...
   - Dashboard nodes include hierarchyLevel property
   - dashboard_syncer.go passes config to NewGraphBuilder
...
<output>
After completion, create `.planning/phases/17-semantic-layer/17-03-SUMMARY.md`
</output>
