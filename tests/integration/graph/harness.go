...
// TestHarness manages a test FalkorDB instance and provides utilities for testing
...
// SeedEvent processes a single event through the pipeline
func (h *TestHarness) SeedEvent(ctx context.Context, event models.Event) error {
	return h.pipeline.ProcessEvent(ctx, event)
}

// SeedEvents processes multiple events through the pipeline
func (h *TestHarness) SeedEvents(ctx context.Context, events []models.Event) error {
	return h.pipeline.ProcessBatch(ctx, events)
}
...
// SeedEventsFromAuditLogFiltered loads and filters events from audit log
func (h *TestHarness) SeedEventsFromAuditLogFiltered(
	ctx context.Context,
	auditLogPath string,
	filter func(models.Event) bool,
) error {
	events, err := LoadAuditLogFiltered(auditLogPath, filter)
	if err != nil {
		return fmt.Errorf("failed to load filtered audit log: %w", err)
	}
	...
	for i := 0; i < len(events); i += batchSize {
		...
		batch := events[i:end]
		if err := h.pipeline.ProcessBatch(ctx, batch); err != nil {
			return fmt.Errorf("failed to process filtered event batch [%d:%d]: %w", i, end, err)
		}
	}
...
func (h *TestHarness) GetConfig() graph.ClientConfig {
	return h.config
}

// GetGraphName returns the unique graph name for this test
func (h *TestHarness) GetGraphName() string {
	return h.graphName
}
...
// Close is an alias for Cleanup for consistency
