# rpk Development Guidelines

Auto-generated from all feature plans. Last updated: 2025-11-25

## Active Technologies
- Go 1.21+ + Standard library (context, sync, os/signal, syscall) (003-graceful-initialization)
- Block-based storage backend (existing in internal/storage) (003-graceful-initialization)
- TypeScript 5.x with React 18+ (004-audit-timeline-ui)
- N/A (frontend only - data sourced from backend API at `/internal/api`) (004-audit-timeline-ui)

- Go 1.21+ (001-k8s-event-monitor)

## Project Structure

```text
src/
tests/
```

## Commands

# Add commands for Go 1.21+

## Code Style

Go 1.21+: Follow standard conventions

## Recent Changes
- 005-ui-api-integration: Added Block-based storage backend (existing in internal/storage)
- 004-audit-timeline-ui: Added TypeScript 5.x with React 18+
- 003-graceful-initialization: Added Go 1.21+ + Standard library (context, sync, os/signal, syscall)


<!-- MANUAL ADDITIONS START -->
<!-- MANUAL ADDITIONS END -->
