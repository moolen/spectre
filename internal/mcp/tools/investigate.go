...
func (t *InvestigateTool) generatePrompts(evidence *InvestigationEvidence, _ *client.TimelineResource, investigationType string) []string {
	prompts := make([]string, 0)

	// Default investigation type
	if investigationType == "" {
		if evidence.CurrentStatus == statusError {
	...
	for _, event := range evidence.Events {
		...
		if strings.Contains(event.Reason, statusError) || strings.Contains(event.Reason, "Failed") {
			errorEventCount++
		}
	}
	...
	if investigationType == "incident" {
		...
		} else if evidence.CurrentStatus == statusWarning {
			prompts = append(prompts,
