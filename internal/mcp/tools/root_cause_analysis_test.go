...
func TestRootCauseAnalysisV2_Schema(t *testing.T) {
	...
	t.Run("Causal chain ordering", func(t *testing.T) {
		assert.Len(t, response.Incident.CausalChain, 2)
		assert.Equal(t, 1, response.Incident.CausalChain[0].StepNumber)
		assert.Equal(t, 2, response.Incident.CausalChain[1].StepNumber)
		assert.Equal(t, "MANAGES", response.Incident.CausalChain[0].RelationshipType)
		assert.Equal(t, "OWNS", response.Incident.CausalChain[1].RelationshipType)
	})
...
// TestEmptyCausalChainHandling validates graceful handling of empty chains
func TestEmptyCausalChainHandling(t *testing.T) {
// Test with symptom-only scenario (minimal chain)
...
// Create minimal symptom-only chain
symptomOnlyChain := []CausalStep{
{
StepNumber: 1,
...
RelationshipStrength: calculateRelationshipFactor(symptomOnlyChain),
ErrorMessageMatch:    calculateErrorMatchFactor(&symptom, &rootCause),
ChainCompleteness:    calculateCompletenessFactor(symptomOnlyChain),
}
...
t.Run("Confidence score", func(t *testing.T) {
assert.Greater(t, score, 0.0, "Score should be > 0 even with minimal chain")
...
t.Run("Causal chain structure", func(t *testing.T) {
assert.Len(t, symptomOnlyChain, 1, "Symptom-only chain should have 1 step")
assert.Equal(t, "SYMPTOM", symptomOnlyChain[0].RelationshipType)
assert.Equal(t, "Pod", symptomOnlyChain[0].Resource.Kind)
})
}
