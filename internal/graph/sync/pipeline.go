...
func (p *pipeline) ProcessBatch(ctx context.Context, events []models.Event) error {
	...
	start := time.Now()
	p.logger.Info("Processing batch of %d events", len(events))

	// Build graph updates
	updates, err := p.builder.BuildFromBatch(ctx, events)
	if err != nil {
		atomic.AddInt64(&p.stats.Errors, 1)
		return fmt.Errorf("failed to build graph updates: %w", err)
	}

	// Apply updates to graph
	for _, update := range updates {
		if err := p.applyGraphUpdate(ctx, update); err != nil {
			p.logger.Warn("Failed to apply update from event %s: %v", update.SourceEventID, err)
			atomic.AddInt64(&p.stats.Errors, 1)
			continue
		}
	}

	// Infer causality if enabled
	...
	p.logger.Info("Processed batch of %d events in %v", len(events), time.Since(start))
	return nil
}
