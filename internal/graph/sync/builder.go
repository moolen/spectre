...
// BuildFromEvent creates graph nodes/edges from a Spectre event
...
func (b *graphBuilder) findPodsMatchingLabels(ctx context.Context, selector map[string]string, namespace string) ([]string, error) {
	...
	// 1. Store labels as indexed properties on ResourceIdentity nodes
	// 2. Use Cypher to query directly for matching labels
	// 3. Support matchExpressions (not just matchLabels)
	//
	...
	b.logger.Debug("Found %d potential Pod matches for selector in namespace %s (label filtering not yet implemented)", len(podUIDs), namespace)

	// Return empty for now to avoid creating incorrect edges
	// TODO: Implement proper label matching or store labels as properties
	return []string{}, nil
}
